    ...
    SubShader {
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                fixed3 normalLocal = normalize(lerp(float3(0,0,1),_Normals_var.rgb,node_156));
...
////// Specular:
                ...
                half3 specularColor = float3(node_165,node_165,node_165);
                half3 directSpecular = attenColor * pow(max(0,dot(halfDirection,normalDirection)),specPow)*specularColor;
...
/////// Diffuse:
                ...
                half3 w = float3(node_237,node_237,node_237)*0.5; // Light wrapping
                half3 NdotLWrap = NdotL * ( 1.0 - w );
                half3 forwardLight = max(fixed3(0.0,0.0,0.0), NdotLWrap + w );
                NdotL = max(0.0,dot( normalDirection, lightDirection ));
                half3 directDiffuse = forwardLight * attenColor;
                fixed3 indirectDiffuse = fixed3(0,0,0);
                indirectDiffuse += gi.indirect.diffuse;
                half3 diffuseColor = node_162;
...
/// Final Color:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                void displacement (inout VertexInput v){
                    half4 node_5098 = _Time;
                    float2 node_6 = (v.texcoord0*2.0);
                    float2 node_161 = (node_6+node_5098.g*float2(0.4,0));
                    float node_15 = 1.0;
                    float4 _DisplacementR_var = tex2Dlod(_DisplacementR,float4(TRANSFORM_TEX(node_161, _DisplacementR),0.0,node_15));
                    float4 node_803 = tex2Dlod(_AORGBHeightA,float4(TRANSFORM_TEX(node_6, _AORGBHeightA),0.0,node_15));
                    v.vertex.xyz += (v.normal*((1.0 - max(_DisplacementR_var.r,node_803.a))*((-1.0)*_Depth)));
...
/// Final Color:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                void displacement (inout VertexInput v){
                    half4 node_8815 = _Time;
                    float2 node_6 = (v.texcoord0*2.0);
                    float2 node_161 = (node_6+node_8815.g*float2(0.4,0));
                    float node_15 = 1.0;
                    float4 _DisplacementR_var = tex2Dlod(_DisplacementR,float4(TRANSFORM_TEX(node_161, _DisplacementR),0.0,node_15));
                    float4 node_803 = tex2Dlod(_AORGBHeightA,float4(TRANSFORM_TEX(node_6, _AORGBHeightA),0.0,node_15));
                    v.vertex.xyz += (v.normal*((1.0 - max(_DisplacementR_var.r,node_803.a))*((-1.0)*_Depth)));
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                void displacement (inout VertexInput v){
                    half4 node_453 = _Time;
                    float2 node_6 = (v.texcoord0*2.0);
                    float2 node_161 = (node_6+node_453.g*float2(0.4,0));
                    float node_15 = 1.0;
                    float4 _DisplacementR_var = tex2Dlod(_DisplacementR,float4(TRANSFORM_TEX(node_161, _DisplacementR),0.0,node_15));
                    float4 node_803 = tex2Dlod(_AORGBHeightA,float4(TRANSFORM_TEX(node_6, _AORGBHeightA),0.0,node_15));
                    v.vertex.xyz += (v.normal*((1.0 - max(_DisplacementR_var.r,node_803.a))*((-1.0)*_Depth)));
            ...
            fixed4 frag(VertexOutput i) : SV_Target {
                ...
                float node_156 = saturate((node_803.a-_DisplacementR_var.r));
                float3 node_162 = lerp(float3(0.4117647,0.3826572,0.3602941),(_Diffuse_var.rgb*node_798.rgb),node_156);
                fixed3 diffColor = node_162;
                float node_165 = (node_156*node_162.r);
                fixed3 specColor = float3(node_165,node_165,node_165);
                half roughness = 1.0 - 10.0;
